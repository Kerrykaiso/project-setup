...
import { createAccountService, loginAccountService, logoutSevice, refreshTokenService } from "../services/auth-service.ts"
...
export const loginController = async(req:Request,res:Response,next:NextFunction) =>{
   try {
     ...
     const {accessToken,refreshToken} = signToken({userId:loginData.userWithoutPassword.userId,sessionId:loginData.session.sessionId},"users")
...
export const refreshTokenController = async(req:Request,res:Response,next:NextFunction)=>{
  try {
   ...
   const {needRefreshToken,sessionData} = await refreshTokenService(payload.sessionId)
   if (needRefreshToken) {
      const {accessToken,refreshToken} = signToken({userId:sessionData.userId,sessionId:sessionData.sessionId},"users")
       await setAuthCookie(res,accessToken,refreshToken)
   }
    const {accessToken} = signToken({userId:sessionData.userId,sessionId:sessionData.sessionId},"users",needRefreshToken)
       await setAuthCookie(res,accessToken,refreshToken)
