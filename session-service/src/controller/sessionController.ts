...
export const createSession =async(call: ServerUnaryCall<SessionRequest, SessionResponse>,callback: sendUnaryData<SessionResponse>)=>{
  ...
  try {
     const result =  sequelize.transaction(async (t)=>{
      ...
      const createdSession = await Sessions.create(sessionData,{transaction:t})
    ...
    export const deleteSession =async(call: ServerUnaryCall<deleteSessionRequest, deleteSessionResponse>,callback: sendUnaryData<deleteSessionResponse>)=>{
      try {
          ...
          callback(null, { success} );
           ...
           callback({ code: grpc.status.NOT_FOUND, message: "Error deleting session" })
            logger.error(error)
      }
    }

    export const getSession=async(call:ServerUnaryCall<getSessionRequest,getSessionResponse>,callback:sendUnaryData<getSessionResponse>)=>{
       try {
         const findSession = await Sessions.findByPk(call.request.sessionId)
         if (!findSession) {
           callback({ code: grpc.status.NOT_FOUND, message: "sessionId not found" })
    ...
    export const updateSession = async(call:ServerUnaryCall<updateSessionRequest,updateSessionResponse>,callback:sendUnaryData<updateSessionResponse>)=>{
      try {
         sequelize.transaction(async(t)=>{
          const updated = await Sessions.update({expiresAt:thirtyDays},{where:{sessionId:call.request.sessionId},transaction:t})
          if (!updated) {
            callback({ code: grpc.status.NOT_FOUND, message: "Error updating session" })
